...
def test_prediction(reg, model_descr, X_test, Y_test, output, prediction_metrics, print_results=True, export_predicted_values=True, export_graphical_results=True, print_stack_trace=False):
    """
    ...
    if export_predicted_values:
        collect_predicted_values(model_descr, Y_test, Y_pred, output)
...
def perform_prediction(model_class, model_descr, default_values, features, output, test_function, print_results=True, export_predicted_values=True, export_graphical_results=True, print_stack_trace=False, imbalance_correction_method=False, **kwargs):
    ...
    print("imbalance_correction_method: ", imbalance_correction_method)
    print(kwargs)
    # =======================================================
    #  Initialization
    # =======================================================
    ...
    try:
        ...
        modelfit = model.fit(X_train, Y_train)  # There is an overflow in matmul if there are 300 layers, activation function is "identity" and solver is 'sgd'.
        print(modelfit)
    except RuntimeWarning:  # If there is an overflow in matmul.
        if print_stack_trace:
            traceback.print_exc()
    ...
    #  Tests
    # =======================================================
    computed_prediction_metrics = test_function(model, model_descr, X_test, Y_test, output[0], print_results, export_predicted_values, export_graphical_results)  # A matmul overflow error cause exit here.
